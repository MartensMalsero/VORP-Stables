    ...
    class BuyHorsesMenu_MenuItem
    {
        ...
        private static void SetupSubMenu(Menu mainMenu, KeyValuePair<string, Dictionary<string, double>> horse_cat)
        {
            ...
            mainMenu.OnMenuOpen += (_menu) =>
            {
                ...
                StablesShop.LoadHorsePreview(_menu.ParentMenu.GetCurrentMenuItem().Index, _menu.CurrentIndex,
                    StablesShop.HorsePed);
            };
            ...
            mainMenu.OnMenuClose += (_menu) => { StablesShop.ExitBuyHorseMode(); };
            ...
            mainMenu.OnItemSelect += (_menu, _menuItem, _itemIndex) =>
            {
                ...
                subMenuConfirmBuy.MenuSubtitle = string.Format(GetConfig.Langs["subTitleConfirmBuy"],
                    GetConfig.Langs[
                        GetConfig.HorseLists.ElementAt(_menu.ParentMenu.GetCurrentMenuItem().Index).Value
                            .ElementAt(_itemIndex).Key],
                    GetConfig.HorseLists.ElementAt(_menu.ParentMenu.GetCurrentMenuItem().Index).Value
                        .ElementAt(_itemIndex).Value.ToString());

                foreach (var _itemInMenu in subMenuConfirmBuy.GetMenuItems())
                {
                    if (_itemInMenu.ItemData == "buttonConfirmYes")
                    {
                        _itemInMenu.Label = string.Format(GetConfig.Langs["ConfirmBuyButton"],
                            GetConfig.HorseLists.ElementAt(_menu.ParentMenu.GetCurrentMenuItem().Index).Value
                                .ElementAt(_itemIndex).Value.ToString());
                    }
                }

                StablesShop.horsecost = GetConfig.HorseLists.ElementAt(_menu.ParentMenu.GetCurrentMenuItem().Index)
                    .Value.ElementAt(_itemIndex).Value;
                StablesShop.horsemodel = GetConfig.HorseLists.ElementAt(_menu.ParentMenu.GetCurrentMenuItem().Index)
                    .Value.ElementAt(_itemIndex).Key;
            };
            ...
            mainMenu.OnIndexChange += async (_menu, _oldItem, _newItem, _oldIndex, _newIndex) =>
            {
                ...
                {
                    await StablesShop.LoadHorsePreview(_menu.ParentMenu.GetCurrentMenuItem().Index, _newIndex,
                        StablesShop.HorsePed);
                }
            };
